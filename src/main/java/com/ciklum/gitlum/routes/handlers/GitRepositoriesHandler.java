...
public class GitRepositoriesHandler {
  ...
  public Mono<ServerResponse> getGitRepositories(final ServerRequest source) {
    ...
    return buildGitRepositories
        ...
        .onErrorMap(
            ...
            e ->
                new GithubUserNotFoundException(
                    String.format("Github user %s not found", request.gitUser())))
        .onErrorMap(
  ...
  private Request buildRequest(final ServerRequest source) {
    ...
    final var pageNum =
        source
            .queryParam(requestProperties.getPageNumberParamKey())
            .orElse(requestProperties.getDefaultPageNumber());
    final var resultsPerPage =
        source
            .queryParam(requestProperties.getResultsPerPageParamValue())
            .orElse(requestProperties.getDefaultResultsPerPage());
    final var token =
        source.headers().header(requestProperties.getAuthorizationHeaderKey()).stream()
